...
import org.openmrs.module.amrsreports.reporting.data.ReasonForEligibilityDataDefinition;
...
public class ReasonForEligibilityDataEvaluator extends BatchedExecutionDataEvaluator<ObsRepresentation> {
	...
	private ReasonForEligibilityDataDefinition definition;
	...
	protected PersonDataDefinition setDefinition(PersonDataDefinition def) {
		definition = (ReasonForEligibilityDataDefinition) def;
		return definition;
	}
	...
	protected boolean doBefore(EvaluationContext context, EvaluatedPersonData c, Cohort cohort) {
		EvaluatedPersonData transferStatusColumn,artDatesColumn;


		try {
			transferStatusColumn = Context.getService(PersonDataService.class).evaluate(new TransferStatusDataDefinition(), context);
		    artDatesColumn = Context.getService(PersonDataService.class).evaluate(new EligibilityForARTDataDefinition(), context);
        } catch (EvaluationException e) {
			log.error("could not evaluate Transfer Status", e);
			return false;
		}
		transferStatus = transferStatusColumn.getData();
        artStartDates = artDatesColumn.getData();
